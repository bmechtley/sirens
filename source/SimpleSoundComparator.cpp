...
namespace Sirens {
    ...
    void SimpleSoundComparator::setFeatureSet(FeatureSet* feature_set) {
        ...
        if (featureSet != NULL) {
            ...
            for (int i = 0; i < feature_list.size(); i++)
                featureComparators[i] = new SimpleFeatureComparator(
                    feature_list[i]
                );
        }
    }
    ...
    double SimpleSoundComparator::compare(
        SimpleSoundComparator* sound_comparator
    ) {
        vector<SimpleFeatureComparator*> other_comparators = 
            sound_comparator->getFeatureComparators();
        ...
        if (other_comparators.size() != featureComparators.size())
            ...
            // Compare each feature against corresponding features from the
            // other sound; sum likelihood.
            for (int i = 0; i < featureComparators.size(); i++) {
                likelihood += featureComparators[i]->compare(
                    other_comparators[i]
                );
            }
